...
beforeEach(() => { resetDefine(); resetRegistry(); resetCleanup(); _resetTransportForTest(); vi.clearAllMocks(); configureTransport(mockSend); });

describe("dedupe + cancel interaction", () => {
  it("two concurrent dispatches with dedupe, then cancel — both resolve null", async () => {
    let runCalls = 0;
    const action = defineAction<{ id: string }, string>({ name: "test.dedupe_cancel", dedupe: true, run: (_args, signal) => { runCalls++; return new Promise<string>((_resolve, reject) => { signal.addEventListener("abort", () => { reject(new DOMException("aborted", "AbortError")); }); }); } });
...
describe("debouncedDispatch leading + flush interaction", () => {
  ...
  it("flush after leading fire dispatches the trailing args immediately", () => {
    ...
    const action = defineAction<string, void>({ name: "test.leading_flush", run: (args) => { runArgs.push(args); return Promise.resolve(); } });
...
describe("transportAction idempotency_key interaction", () => {
  it("transportAction with idempotencyKey: true sends key in command", async () => {
    ...
    const action = transportAction<{ chatID: string }>({ name: "test.transport_idem", idempotencyKey: true, command: ({ chatID }) => ({ type: "send", chat_id: chatID }), error: false });
...
describe("per-call callbacks on deduped dispatches", () => {
  it("both callers' onSuccess fire with the same result", async () => {
    ...
    const action = defineAction<string, string>({ name: "test.dedupe_cb", dedupe: true, run: () => new Promise<string>((r) => { resolveRun = r; }) });
