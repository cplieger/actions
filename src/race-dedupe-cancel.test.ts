...
describe("dedupe + cancel + immediate re-dispatch race", () => {
  it("re-dispatch after cancel should start a fresh run, not collapse onto cancelled", async () => {
    let runCount = 0;
    const action = defineAction<string, string>({ name: "test.dedupe_cancel_redispatch", dedupe: true, error: false, run: (_args, signal) => { runCount++; const myRun = runCount; return new Promise<string>((resolve, reject) => { if (signal.aborted) { reject(new DOMException("aborted", "AbortError")); return; } signal.addEventListener("abort", () => { reject(new DOMException("aborted", "AbortError")); }); setTimeout(() => { resolve(`result-${myRun}`); }, 10); }); } });
  ...
  it("second cancel after re-dispatch still works correctly", async () => {
    let runCount = 0;
    const action = defineAction<string, string>({ name: "test.dedupe_double_cancel", dedupe: true, error: false, run: (_args, signal) => { runCount++; return new Promise<string>((resolve, reject) => { if (signal.aborted) { reject(new DOMException("aborted", "AbortError")); return; } signal.addEventListener("abort", () => { reject(new DOMException("aborted", "AbortError")); }); setTimeout(() => { resolve(`result-${runCount}`); }, 10); }); } });
    const p1 = action.dispatch("x");
    action.cancel();
    await p1;
