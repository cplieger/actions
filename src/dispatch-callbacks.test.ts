...
describe("dispatch callbacks — onSuccess", () => {
  ...
  it("does not fire on error", async () => {
    const onSuccess = vi.fn();
    const action = defineAction({ name: "cb.fail", run: async () => { throw new ActionError("nope"); } });
  ...
  it("throwing in onSuccess is caught and logged", async () => {
    ...
    const result = await action.dispatch({}, { onSuccess: () => { throw new Error("callback boom"); } });
...
describe("dispatch callbacks — onError", () => {
  it("fires with error and args on failure", async () => {
    const onError = vi.fn();
    const action = defineAction({ name: "cb.err", run: async () => { throw new ActionError("bad", { status: 500 }); } });
    await action.dispatch("arg", { onError });
    expect(onError).toHaveBeenCalledWith(expect.objectContaining({ message: "bad", status: 500 }), "arg");
  });

  it("does not fire on cancellation", async () => {
    const onError = vi.fn();
    const action = defineAction({ name: "cb.cancel", run: (_args, signal) => new Promise<void>((_, reject) => { signal.addEventListener("abort", () => { reject(new Error("aborted")); }); }) });
...
describe("dispatch callbacks — onSettled", () => {
  ...
  it("fires on error", async () => {
    const onSettled = vi.fn();
    const action = defineAction({ name: "cb.settled.err", run: async () => { throw new ActionError("fail"); } });
  ...
  it("fires on cancellation", async () => {
    const onSettled = vi.fn();
    const action = defineAction({ name: "cb.settled.cancel", run: (_args, signal) => new Promise<void>((_, reject) => { signal.addEventListener("abort", () => { reject(new Error("aborted")); }); }) });
  ...
  it("fires even when onSuccess callback throws", async () => {
    ...
    await action.dispatch("d", { onSuccess: () => { throw new Error("onSuccess boom"); }, onSettled });
