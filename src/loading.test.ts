...
describe("bindLoadingState — single name", () => {
  it("toggles disabled while action is pending", async () => {
    ...
    const action = defineAction({ name: "test.bind1", run: () => new Promise<string>((r) => { resolveRun = r; }) });
  ...
  it("sets aria-busy by default + clears on completion", async () => {
    let resolveRun: () => void;
    const action = defineAction({ name: "test.bind2", run: () => new Promise<void>((r) => { resolveRun = r; }) });
    const btn = document.createElement("button");
  ...
  it("pendingClass adds + removes a CSS class", async () => {
    let resolveRun: () => void;
    const action = defineAction({ name: "test.bind4", run: () => new Promise<void>((r) => { resolveRun = r; }) });
    const btn = document.createElement("button");
  ...
  it("multiple in-flight instances keep btn disabled until ALL complete", async () => {
    ...
    const action = defineAction({ name: "test.bind7", run: () => new Promise<void>((r) => { resolvers.push(r); }) });
    const btn = document.createElement("button");
  ...
  it("returns an unsubscribe that stops further updates", async () => {
    let resolveRun: () => void;
    const action = defineAction({ name: "test.bind8", run: () => new Promise<void>((r) => { resolveRun = r; }) });
    const btn = document.createElement("button");
...
describe("bindLoadingState — multi-name", () => {
  it("disables while ANY named action is pending", async () => {
    let resolve1!: () => void;
    let resolve2!: () => void;
    const a1 = defineAction({ name: "test.multi1", run: () => new Promise<void>((r) => { resolve1 = r; }) });
    const a2 = defineAction({ name: "test.multi2", run: () => new Promise<void>((r) => { resolve2 = r; }) });
    const btn = document.createElement("button");
