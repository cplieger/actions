...
describe("runWithRetry: no retry on abort even for retry-class errors", () => {
  ...
  it("signal aborted externally before run() throws — no retry", async () => {
    let attempts = 0;
    const action = defineAction<void, string>({ name: "test.abort_no_retry", retryable: retryNetwork, retry: { count: 3, delay: 100 }, error: false, run: async (_args, signal) => { attempts++; if (attempts === 1) { await new Promise<void>((resolve) => { signal.addEventListener("abort", () => { resolve(); }, { once: true }); }); throw new ActionError("network error", { code: "network" }); } return "should not reach"; } });
...
describe("transportAction: spread preserves type field", () => {
  it("idempotencyKey spread preserves the command type field", async () => {
    ...
    const action = transportAction<{ chatID: string }>({ name: "test.type_preserved", idempotencyKey: true, command: ({ chatID }) => ({ type: "cancel" as const, chat_id: chatID }), error: false });
