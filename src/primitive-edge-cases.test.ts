...
describe("dedupe with undefined args", () => {
  it("dedupe: true collapses dispatches when args is undefined", async () => {
    let runCalls = 0;
    const action = defineAction<void, string>({ name: "test.dedupe_undefined_args", dedupe: true, run: () => { runCalls++; return new Promise<string>((r) => setTimeout(() => { r("ok"); }, 10)); } });
...
describe("structuredClone fallback on retry (non-cloneable args)", () => {
  it("retry button uses shallow copy when structuredClone fails (DOM refs)", async () => {
    ...
    const action = defineAction<{ el: typeof el; label: string }, string>({ name: "test.retry_dom_args", run: async (args) => { lastArgs = args; throw new ActionError("fail", { status: 0 }); }, retryable: retryNetwork });
...
describe("cancel after dedupe entry created but before runOnce starts (scope-queued)", () => {
  it("cancel while scope-queued resolves null without running", async () => {
    ...
    let victimRan = false;
    const occupant = defineAction<void, string>({ name: "test.scope_occ", scope: "q", run: () => new Promise<string>((r) => { resolveOccupant = () => r("occ"); }) });
    const victim = defineAction<void, string>({ name: "test.scope_victim", scope: "q", dedupe: true, error: false, run: () => { victimRan = true; return Promise.resolve("v"); } });
